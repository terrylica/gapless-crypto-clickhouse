...
# Fetch with limit
df = gcch.fetch_data("ETHUSDT", "4h", limit=1000)
...
# Specific date range
df = gcch.fetch_data("ETHUSDT", "4h", start="2024-01-01", end="2024-06-30")

# Save to custom directory
df = gcch.fetch_data("SOLUSDT", "1h", limit=500, output_dir="./crypto_data")
```text
...
df = gcch.download("ETHUSDT", "4h", start="2020-01-01", end="2023-12-31")

# Full history (symbol-dependent)
df = gcch.download("SOLUSDT", "1d", start="2020-08-11")  # From listing
```text
...
def prepare_backtest_data(symbol, timeframe, start, end):
    ...
    # Fetch data
    df = gcch.download(symbol, timeframe, start=start, end=end)
...
def analyze_portfolio(symbols, timeframe="1h", period_days=30):
    ...
    for symbol in symbols:
        df = gcch.fetch_data(symbol, timeframe, start=start_date, end=end_date)
...
# Check date range
df = gcch.fetch_data("BTCUSDT", "1h", start="2020-01-01")  # Use earlier start date
```text
...
pip install gapless-crypto-data
import gapless_crypto_clickhouse as gcch
df = gcch.download("BTCUSDT", "1h", limit=1000)
