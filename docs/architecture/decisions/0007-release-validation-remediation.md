...
- Runtime `gcch.__version__` returns "4.0.0" (correct) but help text contradicts
...
assert gcch.__version__ == "4.0.0"
assert "v4.0.0" in gcch.__doc__
```text
...
```bash
uv run python -c "import gapless_crypto_clickhouse as gcch; assert gcch.__version__ == '4.0.0'; assert 'v4.0.0' in gcch.__doc__"
```text
