...
- **UNCHANGED**: File-based API (`gcch.download()`, `gcch.fetch_data()`) remains fully supported
...
# Check version
print(gcch.__version__)  # Should print "4.0.0"

# Existing code works unchanged
df = gcch.download("BTCUSDT", timeframe="1h", start="2024-01-01", end="2024-01-31")
...
| ------------------- | -------------------- | ------------------------- | ----------------- | ---------------------------- |
| **Data Collection** | ✅ `gcch.download()`  | ✅ Same API               | ✅ **Unchanged**  | ✅ New API                   |
| **Gap Filling**     | ✅ `gcch.fill_gaps()` | ✅ Same API               | ✅ **Unchanged**  | ✅ New API                   |
...
```bash
# Verify installation
python -c "import gapless_crypto_clickhouse as gcch; print(gcch.__version__)"
# Expected output: 4.0.0

# Test file-based API (should work unchanged)
python -c "
import gapless_crypto_clickhouse as gcch
df = gcch.download('BTCUSDT', timeframe='1d', start='2024-01-01', end='2024-01-07')
...
df = gcch.download("BTCUSDT", timeframe="1h", start="2024-01-01", end="2024-01-07")
...
A: Yes. You can collect data to CSV files using `gcch.download()`, then ingest into ClickHouse using `ClickHouseBulkLoader.ingest_from_dataframe()`. This hybrid approach is recommended for flexibility.
