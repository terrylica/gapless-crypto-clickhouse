...
def test_both_forms_produce_same_results_fetch_data():
    ...
    df1 = gcc.fetch_data("SOLUSDT", "1d", start="2024-01-01", end="2024-01-02")
    df2 = gcc.fetch_data("SOLUSDT", "1d", start_date="2024-01-01", end_date="2024-01-02")
...
def test_conflict_start_and_start_date_raises_error_fetch_data():
    ...
    with pytest.raises(ValueError, match="Cannot specify both 'start' and 'start_date'"):
        gcc.fetch_data("BTCUSDT", "1h", start="2024-01-01", start_date="2024-01-02")
...
def test_conflict_end_and_end_date_raises_error_fetch_data():
    ...
    with pytest.raises(ValueError, match="Cannot specify both 'end' and 'end_date'"):
        gcc.fetch_data("BTCUSDT", "1h", end="2024-06-30", end_date="2024-07-01")


def test_mixed_legacy_and_new_works():
    """Can use start with end_date (mixed forms)."""
    df = gcc.download("BTCUSDT", "1h", start="2024-01-01", end_date="2024-01-02")
    assert len(df) > 0

    # Or the reverse
    df = gcc.download("BTCUSDT", "1h", start_date="2024-01-01", end="2024-01-02")
    assert len(df) > 0
