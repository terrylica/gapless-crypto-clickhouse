...
- `Skill(itp:semantic-release)` - Automated versioning with Node.js semantic-release v25+, PyPI publishing with Doppler
...
- `Skill(quality-tools:multi-agent-e2e-validation)` - Parallel E2E validation workflow for database refactors (3-layer model: environment → data flow → query interface). Discovered 5 critical bugs (100% failure rate) before release.
- `Skill(quality-tools:multi-agent-performance-profiling)` - 5-agent parallel profiling workflow for bottleneck identification. Proved ClickHouse ingests at 1.1M rows/sec (11x faster than target), revealing download as true bottleneck (90% of time).
